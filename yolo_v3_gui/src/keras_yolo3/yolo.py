...
def detect_video(yolo, video_path, output_path="",csvpath=''):
    import cv2
    ...
    while vid.isOpened():
        ...
        for pred in out_pred:
            outdf = outdf.append(pd.DataFrame([[str(vid.get(1)-1)] + pred + [x_size, y_size]], columns=['frame',"xmin", "ymin", "xmax", "ymax", "label", "confidence", "x_size", "y_size"]))
    ...
    vid.release()
    out.release()
    outdf.to_csv(os.path.join(csvpath, videoname +'.csv'), index=False)
    # yolo.close_session()
